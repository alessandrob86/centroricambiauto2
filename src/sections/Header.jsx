...
export function Header({ current, onNavigate }) {
  ...
  /* Trasparente sopra la hero (solo in home, in cima); solido altrove o scrollando */
  ...
  return (
    <header style={{
      ...
      <div style={{ background: "transparent", color: "var(--char-300)", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
        <Container style={{ display: "flex", alignItems: "center", justifyContent: "space-between", height: "36px", fontSize: "var(--fs-xs)" }}>
          ...
          <span style={{ display: "inline-flex", alignItems: "center", gap: "16px", whiteSpace: "nowrap" }}>
      ...
      <Container style={{ display: "flex", alignItems: "center", justifyContent: "space-between", height: "var(--header-h)" }}>
        <button onClick={() => onNavigate("home")} style={{ background: "none", border: "none", cursor: "pointer", padding: 0, display: "flex" }}>
        ...
        <nav style={{ display: "flex", alignItems: "center", gap: "var(--space-6)" }}>
          {nav.map((n) => (
            <button key={n.id} onClick={() => onNavigate(n.id)} style={{
              ...
              borderBottom: current === n.id ? "2px solid var(--cra-gold)" : "2px solid transparent",
          ...
          <Button variant="primary" size="sm" iconLeft={<Icon name="file-text" size={15} />} onClick={() => onNavigate("contatti")}>
            Preventivo
          </Button>
