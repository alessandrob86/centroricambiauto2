...
/* ============================================================
   GESTIONE — persone, e chi vede cosa.
   ============================================================ */
function Gestione({ isAdmin, zone, setErr }) {
  ...
  if (!isAdmin) return <p className="dip-vuoto">Solo l'amministratore può gestire persone e permessi.</p>;
  if (!dip) return <p className="dip-vuoto">Caricamento…</p>;
  ...
  return (
    <React.Fragment>
      <div className="adm-filtri">
        {[["persone", "Persone"], ["permessi", "Chi vede cosa"],
          ...(isAdmin ? [["manutenzione", "Manutenzione"]] : [])].map(([k, v]) => (
        ...
        <React.Fragment>
          ...
          <div className="dip-tab-scroll">
            <table className="dip-tabella">
              ...
              <tbody>
                {dip.map((d, i) => (
                  <motion.tr key={d.id} {...entra(i)}>
                    <td>{`${d.nome ?? ""} ${d.cognome ?? ""}`.trim()}<br /><span className="dip-sub">{d.email ?? "—"}</span></td>
                    <td>{RUOLI[d.ruolo] ?? d.ruolo}</td>
                    <td>{d.zone?.nome ?? <span className="dip-sub">nessuna</span>}</td>
                    <td>
                      {d.user_id
      ...
      {sezione === "permessi" && (
        <React.Fragment>
          ...
          <p className="dip-regola">
            ...
            cambiarla non avrebbe effetto — e un admin che si toglie «Gestione» non avrebbe più
            modo di rimettersela.
          </p>
          <div className="dip-tab-scroll">
            <table className="dip-permessi">
              <thead>
                <tr>
                  ...
                  <tr key={m.codice}>
                    ...
                    {Object.keys(RUOLI).map((r) => {
                      ...
                      if (r === "admin") {
                        return (
                          <td key={r}>
                            <span className="dip-cella si bloccata" title="L'amministratore vede tutti i moduli: la regola non si applica a lui">
                      ...
                      return (
                        <td key={r}>
                          <button type="button"
                            className={`dip-cella ${acceso ? "si" : "no"} ${scelto ? "scelto" : ""}`}
                            ...
                            {scelto && <i className="dip-cella-punto" />}
                          </button>
                        </td>
                      );
                    })}
      ...
      {sezione === "manutenzione" && isAdmin && <Manutenzione setErr={setErr} />}
    </React.Fragment>
  );
}

/* ============================================================
   MANUTENZIONE — le operazioni che si fanno una volta sola.
   ============================================================ */
function Manutenzione({ setErr }) {
  ...
  const conta = useCallback(() => {
    api.contaMediaDaMigrare()
      .then((n) => { setRimasti(n); setTotale((t) => t ?? n); })
      .catch(() => setErr("Non riesco a contare i media da spostare."));
  }, [setErr]);
  useEffect(() => { conta(); }, [conta]);

  /* Uno scaglione alla volta, finché non ne restano. Un file può pesare
     qualche megabyte: caricarli tutti in una chiamata sola vorrebbe dire
     andare in tempo scaduto a metà strada, senza sapere dove. */
  const avvia = async () => {
    setInCorso(true); setErrori([]);
    try {
      let guardia = 0;
      for (;;) {
        const r = await api.migraMedia(15);
        setRimasti(r.rimasti);
        if (r.errori?.length) setErrori((e) => [...e, ...r.errori]);
        if (r.rimasti === 0 || r.schede === 0 || ++guardia > 40) break;
      }
    } catch (e) {
      setErr(String(e?.message || "Lo spostamento si è interrotto."));
    } finally {
      setInCorso(false);
      conta();
    }
  ...
  const fatti = totale != null && rimasti != null ? totale - rimasti : 0;

  return (
    <React.Fragment>
      <p className="dip-regola">
        Le schede importate dal vecchio portale puntano ancora al suo deposito.
        Finché quel progetto è acceso si vedono; <b>il giorno che lo spegni spariscono</b>.
        Questo le porta a casa, nel deposito privato di questo sito.
      </p>

      <div className="dip-graf-box">
        <h3 className="dip-card-titolo" style={{ marginTop: 0 }}>
          <Icon name="image-plus" size={14} color="var(--cra-red)" /> Media dal vecchio portale
        </h3>

        {rimasti === null ? <p className="dip-sub">Conto…</p> : rimasti === 0 ? (
          <p className="dip-sub">
            <Icon name="check-circle-2" size={14} color="#2E7D4F" />{" "}
            Tutto a casa: nessuna scheda punta più al vecchio deposito.
          </p>
        ) : (
          <React.Fragment>
            <p className="dip-sub">
              <b>{numero(rimasti)}</b> schede hanno ancora foto o allegati sul vecchio portale.
            </p>
            {totale > 0 && (
              <span className="dip-barra-quota" style={{ display: "block", height: 10, margin: "10px 0" }}>
                <motion.span style={{ display: "block", height: "100%", background: "var(--cra-red)" }}
                  animate={{ width: `${(fatti / totale) * 100}%` }} transition={{ duration: 0.4 }} />
              </span>
            )}
            <button className="adm-btn" onClick={avvia} disabled={inCorso}>
              <Icon name={inCorso ? "refresh-cw" : "download"} size={14} />
              {inCorso ? `Sposto… ${numero(fatti)} di ${numero(totale)}` : "Porta i media su questo sito"}
            </button>
            <p className="dip-sub" style={{ marginTop: "8px" }}>
              Si può interrompere e riprendere: quelle già spostate non si ripetono.
            </p>
          </React.Fragment>
        )}
        ...
        {errori.length > 0 && (
          <details className="dip-dettaglio">
            <summary>{errori.length} non sono passate</summary>
            <ul className="dip-sub" style={{ margin: "6px 0 0", paddingLeft: "18px" }}>
              {errori.slice(0, 20).map((e, i) => <li key={i}>{e.motivo}</li>)}
            </ul>
          </details>
        )}
      </div>
